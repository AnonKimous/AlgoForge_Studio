...
graph LR
    ...
    subgraph Algo["算法层"]
        AlgorithmLayer["src/algorithm_management + src/algorithm_support<br/>算法装配、运行入口、调度封装，以及包加载、反射、干预、桥接等支持逻辑"]
    end
...
graph LR
    ...
    Skills["tools/skills<br/>教学示例与引导材料"]
...
### Demo 5: Tempo algorithm render preview

The demo asset for the Tempo algorithm is stored at `demo/tempo/tempo.gif`:

![Tempo algorithm render preview](demo/tempo/tempo.gif)

### Demo 6: Collision algorithm render preview

The demo asset for the collision algorithm is stored at `demo/collison/collision.gif`:
