...
An Anaconda or Miniconda conda environment is mandatory. For the first setup, run:
...
If the `algoforge` environment already exists, start with `conda activate algoforge`. The entry points refuse system Python, bundled Python, and unactivated conda environments.

中文说明见 [`READMEcn.md`](READMEcn.md)。
...
graph LR
    ...
    Skills["doc/skills/algorithmStudio<br/>Tutorial examples and guidance material"]
    ExportBuild["Export/build integration<br/>Export package JSON, generate related assets, call repository Python booters"]
...
Build the mainline, SDK, and debug tool with MSVC:
...
You can also use Ninja + clang:

```bat
python boot\booterNinjaClang.py
```

### 6.2 Build an algorithm package

```bat
python boot\booterNinjaClang.py <algorithm_target_name>
```
...
1. Author the package structure and development notes in Algorithm Studio, then prepare the required algorithm files (cpp, vert/frag, cu)
2. Export the package JSON from Algorithm Studio; source generation and compilation remain repository-side developer steps
3. Compile the algorithm through `boot/booterNinjaClang.py`
4. Test the algorithm in debugTools, including whether it runs and how long it takes
5. Create a runtime instance and mount the algorithm in an external program through `sdk`
6. Inspect results, reflection, and preview
7. Modify the algorithm and keep iterating
...
[`doc/VISION.md`](doc/VISION.md), keeping this README focused on onboarding,
