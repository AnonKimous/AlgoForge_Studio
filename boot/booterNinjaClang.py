...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build AlgoForge with Ninja and Clang, with optional CUDA and PhysX setup."
    )
    parser.add_argument(
        "algorithms",
        nargs="*",
...
def main() -> int:
    ...
    if not arguments.skip_mainline:
        subprocess.run(
            [python, str(ROOT / "buildProject" / "OpenSource" / "build_mainline.py")],
            cwd=ROOT,
            env=environment,
            check=True,
        )
    ...
    if arguments.algorithms:
        for algorithm_name in arguments.algorithms:
            subprocess.run(
                [
                    python,
                    str(ROOT / "buildProject" / "OpenSource" / "build_algorithm.py"),
                    algorithm_name,
                ],
                cwd=ROOT,
                env=environment,
        ...
        subprocess.run(
            [python, str(ROOT / "buildProject" / "build_all_algorithms.py"), "OpenSource"],
            cwd=ROOT,
            env=environment,
            check=True,
        )
