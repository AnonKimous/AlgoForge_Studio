...
try:
    ...
    from ..core.shared import CANVAS_PADDING, COLORS
except ImportError:
    from core.backend import (
        ContainerGroupItem,
        ContainerItem,
...
class AlgorithmStudioPaletteMixin:
    ...
    def _scene_group_label_for_view_mode(self, view_mode: str | None = None) -> str:
        ...
        if normalized == "renderpreview":
            return "renderPreviewScene"
        return "algorithmDevScene"

    def _scene_phase_label_for_view_mode(self, view_mode: str | None = None) -> str:
        normalized = str(view_mode or self.canvas_view_mode).strip().lower()
        if normalized == "graph" or normalized == "interventioner_overview":
            return "exec"
    ...
    def _refresh_scene_tabs(self) -> None:
        active_group = self._scene_group_label_for_view_mode()
        active_tab = self._scene_phase_label_for_view_mode()
        self.scene_group_var.set(active_group)
        self.scene_tab_var.set(active_tab)
        if hasattr(self, "scene_banner_badge") and hasattr(self, "scene_banner_text"):
            if active_group == "algorithmDevScene":
                self.scene_banner_badge.configure(text="algorithmDevScene", bg=COLORS["accent_2"], fg=COLORS["window"])
                self.scene_banner_text.configure(text=f"phase preference: {active_tab}")
                if getattr(self, "scene_banner_frame", None) is not None:
                    ...
                    self.scene_banner_text.configure(bg="#111827")
            elif active_group == "renderPreviewScene":
                self.scene_banner_badge.configure(text="renderPreviewScene", bg=COLORS["accent"], fg=COLORS["window"])
                self.scene_banner_text.configure(text="debugTool / render preview")
                if getattr(self, "scene_banner_frame", None) is not None:
                    self.scene_banner_frame.configure(
                        bg="#10131a",
                        highlightbackground=COLORS["accent"],
                ...
                if getattr(self, "scene_banner_frame", None) is not None:
                    self.scene_banner_frame.configure(
                        bg="#0f1b17",
                        highlightbackground=COLORS["good"],
                        highlightcolor=COLORS["good"],
                    )
                    self.scene_banner_text.configure(bg="#0f1b17")
        ordered_groups = list(self.scene_group_buttons.items())
        for _tab_key, widget in ordered_groups:
            if widget.winfo_manager():
                widget.pack_forget()
        for tab_key, widget in ordered_groups:
            widget.pack(in_=self.scene_tab_primary_row, side="left", padx=(0, 6))
            is_active = tab_key == active_group
            widget.configure(
        ...
        for tab_key, widget in ordered_tabs:
            ...
            if tab_group != active_group:
                continue
            target_row = self.scene_tab_phase_row
    ...
    def _activate_scene_group(self, group_name: str) -> None:
        normalized = str(group_name).strip().lower()
        if normalized == "algorithmdevscene":
        ...
        if normalized == "renderpreviewscene":
            ...
            return
        raise AssertionError(f"Unsupported scene group: {group_name}")
    ...
    def _set_canvas_view_mode(self, view_mode: str, *, log_message: str | None = None) -> None:
        ...
        if normalized == self.canvas_view_mode:
            ...
            return
        self._store_scene_positions_for_view(self.canvas_view_mode)
        ...
        if normalized == "renderpreview":
            self._launch_debugtool_preview_mount()
        self._refresh_all()
    ...
    def _toggle_container_overview(self) -> None:
        if self.canvas_view_mode == "container_overview":
            self._set_canvas_view_mode("graph", log_message="Switched to algorithmDevScene.")
            return
        self._set_canvas_view_mode("container_overview", log_message="Switched to container.")

    def _toggle_decomposer_overview(self) -> None:
        if self.canvas_view_mode == "decomposer_overview":
            self._set_canvas_view_mode("graph", log_message="Switched to algorithmDevScene.")
            return
        self._set_canvas_view_mode("decomposer_overview", log_message="Switched to decomposer.")

    def _toggle_decomposer2container_overview(self) -> None:
        if self.canvas_view_mode == "decomposer2container_overview":
            self._set_canvas_view_mode("graph", log_message="Switched to algorithmDevScene.")
            return
        self._set_canvas_view_mode("decomposer2container_overview", log_message="Switched to d2c.")
        ...
        *,
        ...
        if kind in {"variable", "array"} and not normalized_source_kind and not normalized_source_name:
            if self._current_container_scene_scope() != self._inventory_scene_scope_for_zone("algorithm"):
                self._log("Variable/Array can only be created in container. Drag an existing container from Container Tree to use it here.")
                return
        ...
        if kind == "containerelement":
            ...
            if existing_group is not None:
                self._set_project_node_position(existing_group, x, y)
            ...
            self._set_project_node_position(group, x, y)
            self.selected_container_group_name = name
            self.selected_container_name = None
            self.selected_rule_name = None
            self.selected_reflector_name = None
        ...
        if kind == "decomposer":
            ...
            if existing_rule is not None:
                self._set_project_node_position(existing_rule, x, y)
                ...
                return
            rule = DecomposerRule(name=name, source="", target="", x=x, y=y)
            self.project.decomposer_rules.append(rule)
            self._set_project_node_position(rule, x, y)
            self.selected_rule_name = name
            self.selected_container_name = None
            self.selected_reflector_name = None
            self.selected_stage_name = None
        ...
        if kind == "variable":
            ...
            self._set_project_node_position(container, x, y)
        ...
        if kind == "reflector":
            ...
            if existing_reflector is not None:
                self._set_project_node_position(existing_reflector, x, y)
            ...
            self._set_project_node_position(self.project.reflector_items[-1], x, y)
            self.selected_reflector_name = name
            self.selected_container_name = None
            self.selected_stage_name = None
            self.selected_res_node_name = None
        ...
        if kind == "interventioner" or kind == "stage":
            ...
            if existing_stage is not None:
                self._set_project_node_position(existing_stage, x, y)
                self.selected_stage_name = name
                self.selected_container_name = None
                self.selected_reflector_name = None
                self.selected_res_node_name = None
                self._refresh_all()
                self._log(f"Moved phase node {name}.")
                return
            ...
            self._set_project_node_position(stage, x, y)
            self.selected_stage_name = name
            self.selected_container_name = None
            self.selected_reflector_name = None
            self.selected_res_node_name = None
            self._refresh_all()
            self._log(f"Added phase node {name}.")
            return
        if kind == "resnode":
            ...
            self.project.res_nodes.append(res_node)
            self._set_project_node_position(res_node, x, y)
            self.selected_res_node_name = name
            self.selected_container_name = None
            self.selected_rule_name = None
            self.selected_reflector_name = None
        ...
        if kind == "function":
            ...
            if existing_function is not None:
                self._set_project_node_position(existing_function, x, y)
                ...
                return
            function_item = FunctionFrameItem(
                name=name,
                script="script",
                input_name="in",
                output_name="out",
            ...
            self.project.function_frames.append(function_item)
            self._set_project_node_position(function_item, x, y)
            self.selected_function_name = name
            self.selected_container_name = None
            self.selected_rule_name = None
            self.selected_reflector_name = None
    ...
    def _move_palette_drag_item(self, kind: str, name: str, x: float, y: float) -> None:
        if kind == "containerelement":
            ...
            if item:
                self._set_project_node_position(item, x, y)
