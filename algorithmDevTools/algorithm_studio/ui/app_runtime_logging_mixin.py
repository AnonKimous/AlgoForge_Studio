...
try:
    from ..core.shared import PROJECT_ROOT
except ImportError:
    from core.shared import PROJECT_ROOT
...
class AlgorithmStudioRuntimeLoggingMixin:
    def _initialize_runtime_logging(self) -> None:
        log_dir = PROJECT_ROOT / "algorithmDevTools" / "algorithm_studio" / "runtime_logs"
        log_dir.mkdir(parents=True, exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        self.runtime_log_path = log_dir / f"algorithmDevTools_{timestamp}.log"
