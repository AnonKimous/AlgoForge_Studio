...
class AlgorithmStudioCanvasOverlayMixin:
    ...
    def _all_in_one_scene_color(self, scene_key: str) -> str:
        palette = {
            "exec": COLORS["container"],
            "pretick": COLORS["good"],
            "aftertick": COLORS["accent_2"],
            "renderresult": COLORS["resource"],
            "reflect": COLORS["descriptor"],
            "container": COLORS["good"],
            "decomposer": COLORS["accent_2"],
            "d2c": COLORS["agent"],
            "allinone": COLORS["muted"],
            "helperScene": COLORS["muted"],
        }
        return palette.get(str(scene_key).strip().lower(), COLORS["muted"])

    def _draw_all_in_one_scene_overlays(self, canvas: tk.Canvas) -> None:
        if self.canvas_view_mode != "all_in_one":
            return
        for kind, name, item in self.project.iter_nodes():
            ...
            if not isinstance(positions, dict) or not positions:
                continue
            width = float(getattr(item, "width", 220.0) or 220.0)
            height = float(getattr(item, "height", 160.0) or 160.0)
            for scene_key, entry in sorted(positions.items()):
                if not isinstance(entry, dict):
                    continue
                if str(scene_key).strip().lower() == "allinone":
                    continue
                if "x" not in entry or "y" not in entry:
                    continue
                x = float(entry["x"])
                y = float(entry["y"])
                color = self._all_in_one_scene_color(scene_key)
                overlay_tag = f"allinone_overlay:{kind}:{name}:{scene_key}"
                canvas.create_rectangle(
                    ...
                    width=2,
                    dash=(5, 3),
                    tags=("allinone_overlay", overlay_tag, f"node:{kind}:{name}"),
                )
                canvas.create_text(
                    x + 10,
                    y + 8,
                    ...
                    tags=("allinone_overlay", overlay_tag),
                )
