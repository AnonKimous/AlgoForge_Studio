...
try:
    from ..core.shared import BLUEPRINT_NODE_MIN_HEIGHT, BLUEPRINT_NODE_WIDTH, CANVAS_PADDING, COLORS
except ImportError:
    from core.shared import BLUEPRINT_NODE_MIN_HEIGHT, BLUEPRINT_NODE_WIDTH, CANVAS_PADDING, COLORS
...
class AlgorithmStudioCanvasInteractionMixin:
    ...
    def _restore_dragged_node_position(self, drag_state: dict[str, Any]) -> None:
        ...
        self._set_project_node_position(item, original_x, original_y)
    ...
    def _on_canvas_drag(self, event: tk.Event) -> None:
        ...
        if kind == "container":
            ...
            if rule:
                self._set_project_node_position(rule, float(rule.x) + dx, float(rule.y) + dy)
        elif kind == "reflector":
            item = self._find_reflector(name)
            if item:
                self._set_project_node_position(item, float(item.x) + dx, float(item.y) + dy)
        elif kind == "resnode":
            res_node = self._find_res_node(name)
            if res_node:
                self._set_project_node_position(res_node, float(res_node.x) + dx, float(res_node.y) + dy)
        elif kind == "function":
            ...
            if stage:
                self._set_project_node_position(stage, float(stage.x) + dx, float(stage.y) + dy)
