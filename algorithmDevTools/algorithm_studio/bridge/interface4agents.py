...
COMMAND_SPECS: tuple[AgentCommandSpec, ...] = (
    AgentCommandSpec(
        name="v",
        usage="v add [name] [count] [stride] [x=<x> y=<y>] | v update <name> [count] [stride] [x=<x> y=<y>] | v select <name> | v delete <name>",
    ...
    AgentCommandSpec(
        name="a",
        usage="a add [name] [count] [stride] [x=<x> y=<y>] | a update <name> [count] [stride] [x=<x> y=<y>] | a select <name> | a delete <name>",
    ...
    AgentCommandSpec(
        name="function",
        usage='function add <name> [script_language] [input_name] [output_name] [script] [x=<x> y=<y>] | function update <name> [script_language] [input_name] [output_name] [script] [x=<x> y=<y>] | function select <name> | function delete <name>',
        location="Canvas > fun nodes",
        summary="Function node commands.",
        highlight_target="function",
        aliases=("fun",),
    ),
    AgentCommandSpec(
        name="phase",
        usage='phase add <name> [kind] [functions] [used_variables] [used_arrays] [shader_vertex] [shader_fragment] [x=<x> y=<y>] | phase update <name> [kind] [functions] [used_variables] [used_arrays] [shader_vertex] [shader_fragment] [x=<x> y=<y>] | phase select <name> | phase delete <name>',
        location="Canvas > phase nodes",
        summary="Algorithm phase commands.",
        highlight_target="phase",
        aliases=("stage", "phasenode", "phaseNode".lower()),
    ),
    AgentCommandSpec(
        name="createCosNode",
        usage="createCosNode <name> [x=<x> y=<y>]",
...
SCENE_TARGETS: dict[str, tuple[str, str, str]] = {
    "algorithmdevscene": ("graph", "algorithmDevScene", "algorithmDevScene"),
    "algorithmscene": ("graph", "algorithmDevScene", "algorithmDevScene"),
    "main": ("graph", "algorithmDevScene", "algorithmDevScene"),
    "graph": ("graph", "exec", "exec"),
    "exec": ("graph", "exec", "exec"),
    "helperscene": ("container_overview", "helperScene", "helperScene"),
    "container": ("container_overview", "container", "container"),
    "container_overview": ("container_overview", "container", "container"),
    "containeroverview": ("container_overview", "container", "container"),
    "containerscene": ("container_overview", "container", "container"),
    "decomposer": ("decomposer_overview", "decomposer", "decomposer"),
    "decomposer_overview": ("decomposer_overview", "decomposer", "decomposer"),
    "decomposeroverview": ("decomposer_overview", "decomposer", "decomposer"),
    "decomposerscene": ("decomposer_overview", "decomposer", "decomposer"),
    "reflect": ("reflector_overview", "reflect", "reflect"),
    "reflector": ("reflector_overview", "reflect", "reflect"),
    "reflector_overview": ("reflector_overview", "reflect", "reflect"),
    "reflectoroverview": ("reflector_overview", "reflect", "reflect"),
    "reflectorscene": ("reflector_overview", "reflect", "reflect"),
    "interventioner": ("graph", "exec", "exec"),
    "interventioner_overview": ("graph", "exec", "exec"),
    "interventioneroverview": ("graph", "exec", "exec"),
    "interventionerscene": ("graph", "exec", "exec"),
    ...
    "d2c": ("decomposer2container_overview", "d2c", "d2c"),
    "decomposer2container": ("decomposer2container_overview", "d2c", "d2c"),
    "decomposer2container_overview": ("decomposer2container_overview", "d2c", "d2c"),
    "decomposer2containeroverview": ("decomposer2container_overview", "d2c", "d2c"),
    "d2cscene": ("decomposer2container_overview", "d2c", "d2c"),
}
...
def _parse_optional_float_token(token: str) -> float | None:
    try:
...
def _extract_xy_suffix(args: list[str]) -> tuple[list[str], float | None, float | None]:
    filtered: list[str] = []
    ...
    while index < len(args):
        ...
        lower = token.lower()
        if lower == "at" and index + 2 < len(args):
            maybe_x = _parse_optional_float_token(args[index + 1])
            maybe_y = _parse_optional_float_token(args[index + 2])
        ...
        if lower.startswith("x="):
            maybe_x = _parse_optional_float_token(token[2:])
            if maybe_x is not None:
                x_value = maybe_x
                index += 1
                continue
        if lower.startswith("y="):
            maybe_y = _parse_optional_float_token(token[2:])
        ...
        index += 1
    return filtered, x_value, y_value
...
def build_interface4agents_prompt() -> str:
    lines = [
        ...
        "- Use x=<x> y=<y> to pin a node in the current scene canvas.",
        "",
    ]
    for spec in COMMAND_SPECS:
    ...
    lines.extend(
        [
            ...
            "scene container",
            "createCosNode cosNode x=180 y=120",
            "v add v1 1 4",
            ...
            "a add a1 1 12",
            "function add tide_height pseudocode in out \"height = sin(v1)\"",
            "phase add preTick preTick tide_height v1",
            "```",
        ]
    )
    return "\n".join(lines)
...
def _call(app: Any, method_name: str, *args: Any, **kwargs: Any) -> Any:
...
def _execute_container_command(app: Any, kind: str, args: list[str]) -> str:
    ...
    if subcommand == "add":
        ...
        extra_args, x_value, y_value = _extract_xy_suffix(tail[3:])
        if x_value is not None:
            payload["x"] = x_value
        if y_value is not None:
            payload["y"] = y_value
        if extra_args:
            raise RuntimeError(f"Unsupported {kind[0]} add arguments: {' '.join(extra_args)}")
    ...
    if subcommand == "update":
        ...
        if y_value is not None:
            payload["y"] = y_value
        if extra_args:
            raise RuntimeError(f"Unsupported {kind[0]} update arguments: {' '.join(extra_args)}")
...
def _execute_create_cos_node_command(app: Any, args: list[str]) -> str:
    ...
    tail, x_value, y_value = _extract_xy_suffix(args[1:])
    payload = {
        "tool": "ui_add_node",
...
def _execute_function_command(app: Any, args: list[str]) -> str:
    if not args:
        raise RuntimeError("function requires a subcommand.")
    subcommand = args[0].strip().lower()
    tail = args[1:]
    if subcommand == "add":
        if not tail:
            raise RuntimeError("function add requires a name.")
        payload: dict[str, Any] = {
            "tool": "ui_add_node",
            "kind": "function",
            "name": tail[0],
        ...
        if len(tail) > 1:
            payload["script_language"] = tail[1]
        if len(tail) > 2:
            payload["input_name"] = tail[2]
        if len(tail) > 3:
            payload["output_name"] = tail[3]
        script_tokens, x_value, y_value = _extract_xy_suffix(tail[4:])
        if script_tokens:
            payload["script"] = " ".join(script_tokens)
        if x_value is not None:
            payload["x"] = x_value
        if y_value is not None:
            payload["y"] = y_value
    ...
    if subcommand == "update":
        if not tail:
            raise RuntimeError("function update requires a name.")
        payload = {
            "tool": "ui_update_node",
            "kind": "function",
            "name": tail[0],
    ...
    if subcommand == "select":
        if not tail:
            raise RuntimeError("function select requires a name.")
        return _call(app, "_interface4agents_highlight_node", "function", tail[0].strip())
    if subcommand == "delete":
        if not tail:
            raise RuntimeError("function delete requires a name.")
        payload = {
            "tool": "ui_delete_node",
            "kind": "function",
            "name": tail[0],
        ...
        return _call(app, "_apply_agent_ui_delete_node", payload)
    raise RuntimeError(f"Unsupported function command: {subcommand}")


def _execute_phase_command(app: Any, args: list[str]) -> str:
    if not args:
        raise RuntimeError("phase requires a subcommand.")
    subcommand = args[0].strip().lower()
    tail = args[1:]
    if subcommand == "add":
        if not tail:
            raise RuntimeError("phase add requires a name.")
        payload: dict[str, Any] = {
            "tool": "ui_add_node",
            "kind": "stage",
            "name": tail[0],
        ...
        if len(tail) > 1:
            payload["stage_kind"] = tail[1]
        if len(tail) > 2:
            payload["functions"] = tail[2].split(",")
        if len(tail) > 3:
            payload["used_variables"] = tail[3].split(",")
        if len(tail) > 4:
            payload["used_arrays"] = tail[4].split(",")
        if len(tail) > 5:
            payload["shader_vertex"] = tail[5]
        shader_tokens, x_value, y_value = _extract_xy_suffix(tail[6:])
        if shader_tokens:
            payload["shader_fragment"] = " ".join(shader_tokens)
        if x_value is not None:
            payload["x"] = x_value
        if y_value is not None:
            payload["y"] = y_value
    ...
    if subcommand == "update":
        if not tail:
            raise RuntimeError("phase update requires a name.")
        payload = {
            "tool": "ui_update_node",
            "kind": "stage",
            "name": tail[0],
    ...
    if subcommand == "select":
        if not tail:
            raise RuntimeError("phase select requires a name.")
        return _call(app, "_interface4agents_highlight_node", "interventioner", tail[0].strip())
    if subcommand == "delete":
        if not tail:
            raise RuntimeError("phase delete requires a name.")
        payload = {
            "tool": "ui_delete_node",
            "kind": "stage",
            "name": tail[0],
        ...
        return _call(app, "_apply_agent_ui_delete_node", payload)
    raise RuntimeError(f"Unsupported phase command: {subcommand}")
...
def _execute_highlight_command(app: Any, args: list[str]) -> str:
    ...
    if spec is None:
        ...
        if token in {"container", "containerelement", "decomposer", "reflector", "resnode", "function", "functiontext", "phase", "phasenode", "interventioner", "stage"}:
...
def execute_interface4agents_command(app: Any, tokens: list[str]) -> str:
    ...
    if command == "phase":
        return _execute_phase_command(app, args)
    if command == "hang":
