...
The code in this folder is now physically split into layers:

```text
algorithm_studio/
  algorithm_studio.py   entry point and app wiring
  core/                 shared constants, backend state, approval rules
  bridge/               agent bridge and command protocol
  ui/                   Tk UI shell and mixins
```

`algorithm_studio.py` stays as the launch-facing entry module and imports the layered packages above.
...
- `accessRules.md` in `algorithmDevTools\` controls rule-based chat approval.
