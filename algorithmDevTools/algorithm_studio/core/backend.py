...
try:
    ...
    from core.shared import DEFAULT_TEMPLATE_PATH, PROJECT_ROOT, _resolve_codex_command
...
class ProjectState:
    ...
    @staticmethod
    def _scene_position_key(scene_name: str) -> str:
        key = str(scene_name or "").strip().lower()
        if not key:
            raise ValueError("Scene name is required.")
        return key

    def _scene_position_map(self, item: Any) -> dict[str, dict[str, float]]:
        positions = getattr(item, "scene_positions", None)
        if positions is None:
            raise AssertionError(f"Node does not expose scene positions: {type(item).__name__}")
        return positions

    def scene_position_for(self, item: Any, scene_name: str | None = None) -> tuple[float, float]:
        positions = self._scene_position_map(item)
        if not scene_name:
            return float(getattr(item, "x", 0.0)), float(getattr(item, "y", 0.0))
        key = self._scene_position_key(scene_name)
        entry = positions.get(key)
        if isinstance(entry, dict) and "x" in entry and "y" in entry:
            return float(entry["x"]), float(entry["y"])
        return float(getattr(item, "x", 0.0)), float(getattr(item, "y", 0.0))

    def set_scene_position(self, item: Any, scene_name: str, x: float, y: float) -> None:
        positions = self._scene_position_map(item)
        key = self._scene_position_key(scene_name)
    ...
    def _ui_nodes(self) -> list[dict[str, Any]]:
        ...
        for kind, name, item in self.iter_nodes():
            ...
            scene_positions = getattr(item, "scene_positions", None)
            if scene_positions:
                entry["scene_positions"] = copy.deepcopy(scene_positions)
            if kind == "container":
    ...
    def to_package_json(self, *, include_build_state: bool = True) -> dict[str, Any]:
            ...
            for name, entry in variables.items():
                ...
                project.containers.append(
                    ContainerItem(
                        ...
                        scene_positions=copy.deepcopy(entry.get("scene_positions", entry.get("scenePositions", {})))
                        if isinstance(entry.get("scene_positions", entry.get("scenePositions", {})), dict)
                        else {},
            ...
            if source and target:
                project.decomposer_rules.append(
                    DecomposerRule(
                        ...
                        scene_positions=copy.deepcopy(rule.get("scene_positions", rule.get("scenePositions", {})))
                        if isinstance(rule.get("scene_positions", rule.get("scenePositions", {})), dict)
                        else {},
        ...
        for index, item in enumerate(reflector.get("items", []), start=1):
            ...
            project.reflector_items.append(
                ReflectorItem(
                    ...
                    if isinstance(item.get("scene_positions", item.get("scenePositions", {})), dict)
                    else {},
        ...
        if ui_section:
            ...
            if ui_nodes:
                ...
                for index, entry in enumerate(ui_nodes, start=1):
                    ...
                    item_x = float(entry.get("x", getattr(item, "x", 0.0)))
                    item_y = float(entry.get("y", getattr(item, "y", 0.0)))
                    scene_positions = entry.get("scene_positions", entry.get("scenePositions", {}))
                    if isinstance(scene_positions, dict):
                        item.scene_positions = copy.deepcopy(scene_positions)
                    project.set_scene_position(item, "graph", item_x, item_y)
