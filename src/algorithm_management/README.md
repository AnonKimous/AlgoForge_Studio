...
7. The job-system bridge is treated as part of `runtime_systems` ownership; `job_gpu::Execute` is the management-facing GPU tick entry and `runtime_systems::AlgorithmGpuExecutor` is the internal implementation.
8. `job_cpu::Execute` is the matching CPU tick entry for now, and the work still runs on the main thread through the package's temporary main-thread executor.
...
Package-level support, decomposition, and intervention hooks are surfaced
through `algorithm_management`; the helper implementations still live under
`algorithm_support`, but upper layers should use the management facade.
...
- Capability-level composition may redirect containers between packages; `agent` owns that composition, not the algorithm manager.
- GPU execution is mediated here as a management-layer service through `job_gpu`, and that entry is the same capability as the runtime executor implementation.
