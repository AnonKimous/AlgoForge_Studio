...
std::filesystem::path _AlgorithmPluginPath(const std::string& algorithm_name) {
  const std::filesystem::path base = std::filesystem::path(ALGORITHM_LIBRARY_RESOURCE_ROOT) / algorithm_name;
  const std::filesystem::path candidates[] = {
    base / "Debug" / (algorithm_name + ".dll"),
    base / "Release" / (algorithm_name + ".dll"),
    base / (algorithm_name + ".dll"),
  };
...
bool TryLoadAlgorithmPluginComponents(
  ...
  *out_components = {};
  const std::filesystem::path plugin_path = _AlgorithmPluginPath(algorithm_name);
  ...
  request.algorithm_name = algorithm_name.c_str();
  request.algorithm_library_root = ALGORITHM_LIBRARY_RESOURCE_ROOT;
  request.algorithm_folder = algorithm_name.c_str();
