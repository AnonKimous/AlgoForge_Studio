...
struct PackageDecomposerContainerViewInfo {
  std::string source_container_name;
  uint32_t source_byte_offset{0u};
  uint32_t scalar_bits{0u};
};

struct PackageDecomposerTargetRef {
...
struct PackageDecomposerDescriptionBinding {
  ...
  std::vector<PackageDecomposerTargetRef> target_refs;
...
struct PackageDecomposerViewAliasToken;
bool _ParseViewAliasToken(
  const std::string& text,
  PackageDecomposerViewAliasToken* out_token);
struct PackageDecomposerViewAliasToken {
  std::string name;
  uint32_t explicit_scalar_bits{0u};
  bool has_explicit_scalar_bits{false};
};

bool _TryParsePackedSegmentBitsToken(std::string_view text, uint32_t* out_bits);
...
bool _ParseViewAliasToken(
  ...
  out_token->explicit_scalar_bits = 0u;
  out_token->has_explicit_scalar_bits = false;

  const std::string trimmed = _TrimText(text);
  if (trimmed.empty()) {
    return false;
  }
  ...
  out_token->name = _TrimText(trimmed.substr(0, slash));
  if (out_token->name.empty()) {
    return false;
  }

  const std::string bits_text = _TrimText(trimmed.substr(slash + 1u));
  if (!_TryParsePackedSegmentBitsToken(bits_text, &out_token->explicit_scalar_bits)) {
    return false;
  }
  out_token->has_explicit_scalar_bits = true;
  return true;
}
  ...
  if (aliases) {
    ...
    for (int i = 0; i < alias_count; ++i) {
      ...
      if (left_tokens.empty() || right_tokens.empty()) {
        _SetErrorMessage(
          out_error_message,
          "Package container alias must contain both source and target names: " + alias_text +
            " (" + package_path + ")");
        return false;
      }
      ...
      std::vector<PackageDecomposerViewAliasToken> right_view_tokens{};
      right_view_tokens.reserve(right_tokens.size());
      bool has_explicit_view_width = false;
      for (const std::string& raw_right_token : right_tokens) {
        PackageDecomposerViewAliasToken view_token{};
        if (!_ParseViewAliasToken(raw_right_token, &view_token)) {
          _SetErrorMessage(
            out_error_message,
            "Package container alias contains an invalid target token: " + raw_right_token +
              " (" + package_path + ")");
          return false;
        }
        has_explicit_view_width = has_explicit_view_width || view_token.has_explicit_scalar_bits;
        right_view_tokens.push_back(std::move(view_token));
      }
      if (has_explicit_view_width) {
        for (const PackageDecomposerViewAliasToken& view_token : right_view_tokens) {
          if (!view_token.has_explicit_scalar_bits) {
            _SetErrorMessage(
              out_error_message,
              "Package container view alias cannot mix explicit and implicit widths: " + alias_text +
                " (" + package_path + ")");
            return false;
          }
        }
      ...
      const auto is_name_conflict = [&](const std::string& name) {
        return out_schema->container_infos.find(name) != out_schema->container_infos.end() ||
          out_schema->container_aliases_by_name.find(name) != out_schema->container_aliases_by_name.end() ||
          out_schema->container_view_aliases_by_name.find(name) != out_schema->container_view_aliases_by_name.end();
      };

      if (left_tokens.size() == 1u && (right_view_tokens.size() > 1u || has_explicit_view_width)) {
        const std::string source_name = _TrimText(left_tokens.front());
        if (source_name.empty()) {
          _SetErrorMessage(out_error_message, "Package container alias contains an empty source name: " + package_path);
          return false;
        }
        const auto source_found = out_schema->container_infos.find(source_name);
        if (source_found == out_schema->container_infos.end()) {
          _SetErrorMessage(
            out_error_message,
            "Package container view alias references an unknown container name: " + source_name +
              " (" + package_path + ")");
          return false;
        }
        ...
        if (has_explicit_view_width) {
          for (const PackageDecomposerViewAliasToken& view_token : right_view_tokens) {
            if (view_token.explicit_scalar_bits == 0u) {
              _SetErrorMessage(
                out_error_message,
                "Package container view alias requires explicit widths for every segment: " + alias_text +
                  " (" + package_path + ")");
              return false;
            }
            if (segment_bits > UINT32_MAX - view_token.explicit_scalar_bits) {
              _SetErrorMessage(
                out_error_message,
                "Package container view alias width sum overflowed: " + alias_text +
                  " (" + package_path + ")");
              return false;
            }
            segment_bits += view_token.explicit_scalar_bits;
          }
          if (segment_bits > source_found->second.scalar_bits) {
            _SetErrorMessage(
              out_error_message,
              "Package container view alias exceeds the source precision: " + alias_text +
                " (" + package_path + ")");
            return false;
          }
        } else {
          if (source_found->second.scalar_bits == 0u ||
              source_found->second.scalar_bits % right_view_tokens.size() != 0u) {
            _SetErrorMessage(
              out_error_message,
              "Package container view alias cannot split source precision evenly: " + alias_text +
                " (" + package_path + ")");
            return false;
          }
          segment_bits = source_found->second.scalar_bits / static_cast<uint32_t>(right_view_tokens.size());
        }
        if (segment_bits == 0u || segment_bits % 8u != 0u) {
          _SetErrorMessage(
            out_error_message,
            "Package container view alias requires byte-aligned split widths: " + alias_text +
              " (" + package_path + ")");
          return false;
        }
        ...
        uint32_t byte_offset = 0u;
        for (const PackageDecomposerViewAliasToken& view_token : right_view_tokens) {
          if (view_token.name.empty()) {
            _SetErrorMessage(out_error_message, "Package container alias name is empty: " + package_path);
            return false;
          }
          if (is_name_conflict(view_token.name)) {
            _SetErrorMessage(
              out_error_message,
              "Package container alias conflicts with an existing container or alias name: " + view_token.name +
                " (" + package_path + ")");
            return false;
          }
          if (!unique_alias_names.insert(view_token.name).second) {
            _SetErrorMessage(
              out_error_message,
              "Package container alias repeats the same alias more than once: " + view_token.name +
                " (" + package_path + ")");
            return false;
          }
          const uint32_t view_bits = has_explicit_view_width ? view_token.explicit_scalar_bits : segment_bits;
          out_schema->container_aliases_by_name.emplace(view_token.name, std::vector<std::string>{source_name});
          out_schema->container_view_aliases_by_name.emplace(
            view_token.name,
            PackageDecomposerContainerViewInfo{
              .source_container_name = source_name,
              .source_byte_offset = byte_offset,
              .scalar_bits = view_bits,
            });
          byte_offset += view_bits / 8u;
        }
        continue;
      }

      const std::string alias_name = _TrimText(right_view_tokens.front().name);
      if (alias_name.empty()) {
        _SetErrorMessage(out_error_message, "Package container alias name is empty: " + package_path);
        return false;
      }
      ...
      for (const std::string& token_raw : left_tokens) {
        const std::string token = _TrimText(token_raw);
        if (token.empty()) {
          _SetErrorMessage(
            out_error_message,
        ...
        if (out_schema->container_infos.find(token) != out_schema->container_infos.end()) {
          ...
          const auto alias_found = out_schema->container_aliases_by_name.find(token);
          if (alias_found == out_schema->container_aliases_by_name.end() || alias_found->second.size() != 1u) {
            _SetErrorMessage(
              out_error_message,
          ...
          resolved_name = alias_found->second.front();
        }
...
bool _TryResolveTargetContainerReference(
  const PackageDecomposerSchema& schema,
  std::string_view container_name,
  PackageDecomposerTargetRef* out_target_ref) {
  if (!out_target_ref) {
    return false;
  }
  out_target_ref->target_name.clear();
  out_target_ref->container_name.clear();
  out_target_ref->byte_offset = 0u;
  out_target_ref->scalar_bits = 0u;
  if (container_name.empty()) {
    return false;
  }
  ...
  if (container_info) {
    out_target_ref->target_name = std::string(container_name);
    out_target_ref->container_name = std::string(container_name);
    out_target_ref->scalar_bits = container_info->scalar_bits;
    return true;
  }

  const auto view_found = schema.container_view_aliases_by_name.find(std::string(container_name));
  if (view_found != schema.container_view_aliases_by_name.end()) {
    const PackageDecomposerContainerViewInfo& view_info = view_found->second;
    const PackageDecomposerContainerInfo* source_info = _FindContainerInfo(schema, view_info.source_container_name);
    if (!source_info) {
      return false;
    }
    out_target_ref->target_name = std::string(container_name);
    out_target_ref->container_name = view_info.source_container_name;
    out_target_ref->byte_offset = view_info.source_byte_offset;
    out_target_ref->scalar_bits = view_info.scalar_bits;
    return true;
  }
  ...
  if (alias_found->second.size() != 1u) {
    return false;
  }
  const std::string& resolved_name = alias_found->second.front();
  const PackageDecomposerContainerInfo* resolved_info = _FindContainerInfo(schema, resolved_name);
  if (!resolved_info) {
    return false;
  }
  out_target_ref->target_name = std::string(container_name);
  out_target_ref->container_name = resolved_name;
  out_target_ref->scalar_bits = resolved_info->scalar_bits;
  return true;
}
...
bool _ExpandResolvedTargetRefs(
  const PackageDecomposerSchema& schema,
  const std::vector<std::string>& raw_names,
  const std::string& package_path,
  std::vector<PackageDecomposerTargetRef>* out_target_refs,
  std::string* out_error_message) {
  if (!out_target_refs) {
    _SetErrorMessage(out_error_message, "Target reference expansion output vector is null.");
    return false;
  }
  out_target_refs->clear();

  for (const std::string& raw_name : raw_names) {
    PackageDecomposerTargetRef resolved_ref{};
    if (_TryResolveTargetContainerReference(schema, raw_name, &resolved_ref)) {
      out_target_refs->push_back(std::move(resolved_ref));
      continue;
    }

    const auto alias_found = schema.container_aliases_by_name.find(raw_name);
    if (alias_found == schema.container_aliases_by_name.end()) {
      _SetErrorMessage(
        out_error_message,
        "Package decomposer target references an unknown container name '" + raw_name +
          "': " + package_path);
      return false;
    }
    for (const std::string& resolved_name : alias_found->second) {
      PackageDecomposerTargetRef alias_ref{};
      alias_ref.target_name = raw_name;
      alias_ref.container_name = resolved_name;
      const PackageDecomposerContainerInfo* container_info = _FindContainerInfo(schema, resolved_name);
      if (!container_info) {
        _SetErrorMessage(
          out_error_message,
          "Package decomposer target alias references an unknown container name '" + resolved_name +
            "': " + package_path);
        return false;
      }
      alias_ref.scalar_bits = container_info->scalar_bits;
      out_target_refs->push_back(std::move(alias_ref));
    }
  }

  if (out_target_refs->empty()) {
    _SetErrorMessage(
      out_error_message,
      "Package decomposer target references resolve to an empty list: " + package_path);
    return false;
  }
  return true;
}
...
bool _ParsePackageDecomposerBinding(
  ...
  if (to_item && cJSON_IsObject(to_item)) {
    ...
    if (!_ExpandResolvedTargetRefs(
          schema,
          json_utils::GetStringList(to_item),
          package_path,
          &out_binding->target_refs,
          out_error_message)) {
      return false;
    }
  }
  ...
  const size_t target_count = !out_binding->target_container_names.empty()
    ...
    : !out_binding->target_refs.empty()
      ? out_binding->target_refs.size()
...
const agentmanager::agent::AlgorithmDescriptorValue* _FindDescriptorValue(
  ...
  for (const agentmanager::agent::AlgorithmDescriptorValue& value : descriptor_values) {
...
bool _ExtractPackedDescriptorSegments(
  const agentmanager::agent::AlgorithmDescriptorValue& descriptor_value,
...
const agentmanager::agent::AlgorithmResourceBinding* _FindResourceBinding(
 ...
 public:
    ...
    *out_requested_descriptor_bindings = {};
    ...
    for (const PackageDecomposerDescriptionEntry& entry : schema_.description_entries) {
      for (const PackageDecomposerDescriptionBinding& binding : entry.bindings) {
        if (!binding.target_refs.empty()) {
          if (binding.from_names.size() != binding.target_refs.size() &&
          ...
          for (size_t i = 0; i < binding.target_refs.size(); ++i) {
            ...
            out_requested_descriptor_bindings->descriptor_slots.push_back(
              agentmanager::agent::AlgorithmRequestedDescriptorBindings::DescriptorSlot{
  ...
  bool DecomposeDescriptors(
    ...
    for (const PackageDecomposerDescriptionEntry& entry : schema_.description_entries) {
      for (const PackageDecomposerDescriptionBinding& binding : entry.bindings) {
        if (!binding.target_refs.empty()) {
          if (!WriteDescriptorTargetsByRef(
                algorithm_profile,
                entry,
                binding,
                descriptor_values,
 ...
 private:
  bool WriteDescriptorTargetsByName(
    ...
    for (size_t i = 0; i < target_count; ++i) {
      ...
      const agentmanager::agent::AlgorithmDescriptorValue* value =
  ...
  bool WriteDescriptorTargetsByRef(
    const algorithm::AlgorithmProfile& algorithm_profile,
    const PackageDecomposerDescriptionEntry& entry,
    const PackageDecomposerDescriptionBinding& binding,
    const std::vector<algorithmManager::AlgorithmDescriptorValue>& descriptor_values,
    ...
    std::string* out_error_message) const {
    const size_t from_count = binding.from_names.size();
    const size_t target_count = binding.target_refs.size();
    if (from_count != target_count && from_count != 1u) {
      _SetErrorMessage(
        out_error_message,
        "Description entry '" + entry.name + "' has mismatched from/to list sizes for '" +
    ...
    for (size_t i = 0; i < target_count; ++i) {
      ...
      const PackageDecomposerTargetRef& target_ref = binding.target_refs[i];
      algorithm::AlgorithmContainer* container = FindAlgorithmContainer(container_set, target_ref.container_name);
      if (!container) {
        _SetErrorMessage(
          out_error_message,
          "Missing target container '" + target_ref.container_name + "' for '" +
            algorithm_profile.algorithm_name + "'.");
        return false;
      }
      const PackageDecomposerContainerInfo* container_info = _FindContainerInfo(schema_, target_ref.container_name);
      if (!container_info) {
        _SetErrorMessage(
          out_error_message,
          "Missing target container schema '" + target_ref.container_name + "' for '" +
            algorithm_profile.algorithm_name + "'.");
        return false;
      }
      ...
      if (!_WriteDescriptorScalarValue(
        ...
        if (out_error_message && out_error_message->empty()) {
          _SetErrorMessage(
            out_error_message,
            "Target container '" + target_ref.container_name + "' could not accept descriptor value for '" +
              algorithm_profile.algorithm_name + "'.");
        }
        return false;
      }
  ...
  bool PreparePackedSegmentValues(
    ...
    const agentmanager::agent::AlgorithmDescriptorValue* source_value =
...
void _CopyBridgeDebugBindings(
  ...
  for (const algorithm::AlgorithmRuntimeTransferBinding& binding : edge->bindings) {
    out_bindings->push_back(algorithmManager::scheduler::PipelineStageBridgeDebugBinding{
...
bool _ReadJobsInterStageScalarAt(
  const algorithmManager::JobsPipelineInterStageBufferRuntimeState& inter_stage_buffer,
  uint32_t index,
  float* out_value,
  std::string* out_error_message) {
...
bool _WriteJobsInterStageScalarAt(
  algorithmManager::JobsPipelineInterStageBufferRuntimeState* inter_stage_buffer,
  uint32_t index,
  float value,
  std::string* out_error_message) {
