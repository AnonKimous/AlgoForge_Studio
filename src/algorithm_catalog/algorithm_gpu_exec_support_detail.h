...
namespace algorithmManager::catalog::vk_exec_detail {

namespace fs = std::filesystem;

struct VkExecSchema {
  agentmanager::agent::AlgorithmVkExecSpec spec{};
  bool valid{false};
...
inline void LoadVkExecBindings(
  ...
  std::vector<agentmanager::agent::AlgorithmVkExecContainerBinding>* out_bindings,
  const std::string& package_path,
  std::string* out_error_message) {
  if (!out_bindings) {
    if (out_error_message) {
      *out_error_message = "VK exec bindings output pointer is null.";
    }
    return;
  }
  ...
  const auto append_binding_group = [&](const cJSON* items, const char* default_kind, uint32_t default_tuple_width) {
    ...
    for (int i = 0; i < container_count; ++i) {
      ...
      agentmanager::agent::AlgorithmVkExecContainerBinding binding{};
      ...
      if (binding.container_name.empty()) {
        if (out_error_message) {
          *out_error_message = "Invalid VK exec container binding in package JSON file: " + package_path;
        }
        return false;
      }
...
inline VkExecSchema LoadVkExecSchema(
  const algorithm::AlgorithmPackageLocation& package_location) {
  VkExecSchema schema{};
...
class JsonAlgorithmVkExecutor final : public agentmanager::agent::IAlgorithmVkExecutor {
 public:
  explicit JsonAlgorithmVkExecutor(agentmanager::agent::AlgorithmVkExecSpec spec)
    : spec_(std::move(spec)) {}

  bool GetVkExecSpec(agentmanager::agent::AlgorithmVkExecSpec* out_spec) const override {
    if (!out_spec) {
      return false;
    }
    *out_spec = spec_;
 ...
 private:
  agentmanager::agent::AlgorithmVkExecSpec spec_{};
};
...
inline bool LoadAlgorithmVkExecutorFromLocationImpl(
  ...
  if (!out_vk_executor) {
    if (out_error_message) {
      *out_error_message = "VK executor output pointer is null.";
    }
    return false;
  }

  const VkExecSchema schema = LoadVkExecSchema(package_location);
  if (!schema.valid) {
  ...
  *out_vk_executor = std::make_shared<JsonAlgorithmVkExecutor>(schema.spec);
  if (out_error_message) {
    out_error_message->clear();
  }
  return true;
