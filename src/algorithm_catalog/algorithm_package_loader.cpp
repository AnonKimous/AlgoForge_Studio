...
#include "algorithm_catalog/algorithm_vk_exec_support_detail.h"
#include "algorithm_catalog/algorithm_intervention_support_detail.h"
...
algorithmManager::catalog::AlgorithmPipelineWrapperStageSpec _LoadWrapperStageSpec(
  const cJSON* wrapper_stage_object,
  const char* stage_key) {
  algorithmManager::catalog::AlgorithmPipelineWrapperStageSpec stage_spec{};
...
bool CreateAlgorithmObjectFromLocation(
  ...
  agentmanager::agent::AlgorithmObject group{};
  ...
  std::shared_ptr<agentmanager::agent::IAlgorithmIntervention> package_intervention{};
  ...
  std::shared_ptr<agentmanager::agent::IAlgorithmVkExecutor> package_vk_executor{};
  ...
  const bool allow_reflector =
    load_reflector &&
    algorithm::library_paths::GetAlgorithmLibraryRuntimeBuildFlavor() !=
...
bool TryLoadAlgorithmPluginComponents(
  ...
  *out_components = {};
  ...
  if (bundle.api_version != algorithmManager::catalog::kAlgorithmPluginApiVersion) {
    _SetErrorMessage(
      out_error_message,
...
bool TryBuildAlgorithmInterventionVkPhaseSubJob(
  ...
  for (const ::algorithmManager::scheduler::AlgorithmPhaseContainerBinding& binding : phase_spec.used_algorithm_containers) {
    ::algorithm::AlgorithmContainer* container =
    ...
    runtime_systems::RuntimeVkBufferBindingView binding_view{};
...
bool TryBuildAlgorithmVkExecStageSubJob(
  ...
  for (const ::algorithmManager::scheduler::AlgorithmVkExecContainerBinding& binding : vk_exec_spec.used_algorithm_containers) {
    ::algorithm::AlgorithmContainer* container =
      ::algorithm::FindAlgorithmContainer(container_set, binding.container_name);
    if (!container) {
      if (out_error_message) {
...
#include "algorithm_management/algorithm_manager.cpp"
