...
- `runtimesys/runtime_systems.h` is the only public facade for this layer
- `RuntimeEnvironment`
- runtime job entry points are exposed through the facade for upper layers that need them
- VK job execution lives in the `vk_job_system.cpp` implementation and is exposed through the job facade.
...
- `job_system.h`, `vk_job_system.h`, `runtime_environment.h`, and the rendering/window implementation headers are internal implementation headers.
