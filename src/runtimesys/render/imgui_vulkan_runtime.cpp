...
void AppendImguiVulkanProbe(const std::string& line) {
  ...
  const std::filesystem::path probe_path =
    RuntimeDebugInfoRoot() / "runtime_init_probe.log";
  std::error_code ec;
...
void ImGuiVulkanRuntime::CreateSwapchainCaptureBuffers() {
  if (!swapchain_readback_enabled_) {
    return;
  }

  switch (main_window_.SurfaceFormat.format) {
    case VK_FORMAT_B8G8R8A8_UNORM:
    case VK_FORMAT_B8G8R8A8_SRGB:
    case VK_FORMAT_R8G8B8A8_UNORM:
    case VK_FORMAT_R8G8B8A8_SRGB:
      swapchain_capture_bytes_per_pixel_ = 4u;
      break;
    case VK_FORMAT_B8G8R8_UNORM:
    case VK_FORMAT_B8G8R8_SRGB:
    case VK_FORMAT_R8G8B8_UNORM:
    case VK_FORMAT_R8G8B8_SRGB:
      swapchain_capture_bytes_per_pixel_ = 3u;
      break;
    default:
      throw std::runtime_error("Presented swapchain format cannot be captured.");
  }

}

void ImGuiVulkanRuntime::DestroySwapchainCaptureBuffers() {
  swapchain_capture_bytes_per_pixel_ = 0u;
}

void ImGuiVulkanRuntime::RecordSwapchainCapture(VkCommandBuffer command_buffer, VkImage image) {
  if (!debug_tool_recording_active_) {
    return;
  }

  SwapchainCaptureFrame capture{};
  capture.width = static_cast<uint32_t>(main_window_.Width);
  capture.height = static_cast<uint32_t>(main_window_.Height);
  capture.bytes_per_pixel = swapchain_capture_bytes_per_pixel_;
  capture.format = main_window_.SurfaceFormat.format;
  capture.tick_sequence = preview_frame_sequence_;
  VkBufferCreateInfo buffer_info{};
  buffer_info.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
  buffer_info.size =
    static_cast<VkDeviceSize>(capture.width) *
    static_cast<VkDeviceSize>(capture.height) *
    static_cast<VkDeviceSize>(capture.bytes_per_pixel);
  ...
  CheckVkResult(vmaCreateBuffer(
    ...
    &capture.buffer,
    &capture.allocation,
    &capture.allocation_info));
  recorded_swapchain_frames_.push_back(std::move(capture));
  SwapchainCaptureFrame& recorded_frame = recorded_swapchain_frames_.back();
  ...
  copy_region.imageExtent = {
    static_cast<uint32_t>(main_window_.Width),
...
void ImGuiVulkanRuntime::RecordResultPreviewCapture(VkCommandBuffer command_buffer) {
  RecordPreviewImageCapture(
    command_buffer,
    result_preview_copy_.image,
    result_preview_copy_.extent,
    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
}

void ImGuiVulkanRuntime::RecordPreviewImageCapture(
  ...
  copy_region.imageSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
  copy_region.imageSubresource.layerCount = 1u;
  copy_region.imageExtent = {capture.width, capture.height, 1u};
  vkCmdCopyImageToBuffer(
    command_buffer,
    image,
    VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
  ...
  to_shader_read.newLayout = image_layout;
  vkCmdPipelineBarrier(
    command_buffer,
    VK_PIPELINE_STAGE_TRANSFER_BIT,
    VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
...
void ImGuiVulkanRuntime::CreateResultPreviewCopy(VkExtent2D extent) {
  ...
  CheckVkResult(vmaCreateImage(
    ...
    &allocation_info,
    &result_preview_copy_.image,
    &result_preview_copy_.allocation,
    nullptr));

  VkImageViewCreateInfo view_info{};
  view_info.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
  view_info.image = result_preview_copy_.image;
  ...
  CheckVkResult(vkCreateSampler(
    ...
    &sampler_info,
    allocator_,
    &result_preview_copy_.sampler));
  result_preview_copy_.extent = extent;
}

void ImGuiVulkanRuntime::DestroyResultPreviewCopy() {
  if (result_preview_copy_.sampler != VK_NULL_HANDLE) {
    vkDestroySampler(device_, result_preview_copy_.sampler, allocator_);
    result_preview_copy_.sampler = VK_NULL_HANDLE;
  }
  if (result_preview_copy_.view != VK_NULL_HANDLE) {
    vkDestroyImageView(device_, result_preview_copy_.view, allocator_);
    result_preview_copy_.view = VK_NULL_HANDLE;
  }
  if (result_preview_copy_.image != VK_NULL_HANDLE) {
    vmaDestroyImage(
      vma_allocator_,
      result_preview_copy_.image,
      result_preview_copy_.allocation);
    result_preview_copy_.image = VK_NULL_HANDLE;
    result_preview_copy_.allocation = VK_NULL_HANDLE;
  }
  result_preview_copy_.extent = {};
  result_preview_copy_.initialized = false;
}

void ImGuiVulkanRuntime::RecordResultPreviewCopy(VkCommandBuffer command_buffer) {
  VkImageMemoryBarrier source_to_transfer{};
  source_to_transfer.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
  source_to_transfer.srcAccessMask =
    VK_ACCESS_SHADER_READ_BIT |
    VK_ACCESS_SHADER_WRITE_BIT |
    VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
  source_to_transfer.dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT;
  source_to_transfer.oldLayout = result_image_.layout;
  source_to_transfer.newLayout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
  source_to_transfer.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
  source_to_transfer.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
  source_to_transfer.image = result_image_.image;
  source_to_transfer.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
  source_to_transfer.subresourceRange.levelCount = 1u;
  source_to_transfer.subresourceRange.layerCount = 1u;

  VkImageMemoryBarrier copy_target_to_transfer{};
  copy_target_to_transfer.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
  copy_target_to_transfer.srcAccessMask = result_preview_copy_.initialized
    ? VK_ACCESS_SHADER_READ_BIT
    : 0u;
  copy_target_to_transfer.dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
  copy_target_to_transfer.oldLayout = result_preview_copy_.initialized
    ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
    : VK_IMAGE_LAYOUT_UNDEFINED;
  copy_target_to_transfer.newLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
  copy_target_to_transfer.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
  copy_target_to_transfer.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
  copy_target_to_transfer.image = result_preview_copy_.image;
  copy_target_to_transfer.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
  copy_target_to_transfer.subresourceRange.levelCount = 1u;
  copy_target_to_transfer.subresourceRange.layerCount = 1u;

  vkCmdPipelineBarrier(
    command_buffer,
    result_preview_copy_.initialized
      ? VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT |
    ...
    nullptr,
    2u,
    std::array<VkImageMemoryBarrier, 2>{
      source_to_transfer,
      copy_target_to_transfer}.data());
  ...
  copy_region.extent = {result_preview_copy_.extent.width, result_preview_copy_.extent.height, 1u};
  vkCmdCopyImage(
    command_buffer,
    result_image_.image,
    VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
  ...
  VkImageMemoryBarrier source_to_shader = source_to_transfer;
  ...
  source_to_shader.newLayout = result_image_.layout;

  VkImageMemoryBarrier copy_target_to_shader = copy_target_to_transfer;
  copy_target_to_shader.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
  copy_target_to_shader.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
  copy_target_to_shader.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
  copy_target_to_shader.newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
  vkCmdPipelineBarrier(
    command_buffer,
    VK_PIPELINE_STAGE_TRANSFER_BIT,
    VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
    ...
    std::array<VkImageMemoryBarrier, 2>{
      source_to_shader,
      copy_target_to_shader}.data());
  result_preview_copy_.initialized = true;
}
...
void ImGuiVulkanRuntime::SetupVulkanWindow(SDL_Window* window, int width, int height) {
  ...
  if (swapchain_readback_enabled_) {
    ...
    if ((surface_capabilities.supportedUsageFlags & VK_IMAGE_USAGE_TRANSFER_SRC_BIT) == 0u) {
      throw std::runtime_error("Presented swapchain does not support transfer-source capture.");
    }
  }
  ...
  ImGui_ImplVulkanH_CreateOrResizeWindow(
    ...
    height,
    min_image_count_,
    swapchain_readback_enabled_ ? VK_IMAGE_USAGE_TRANSFER_SRC_BIT : 0);
  CreateSwapchainCaptureBuffers();
}

void ImGuiVulkanRuntime::CleanupVulkanWindow() {
  DestroySwapchainCaptureBuffers();
  for (SwapchainCaptureFrame& recorded_frame : recorded_swapchain_frames_) {
    vmaDestroyBuffer(vma_allocator_, recorded_frame.buffer, recorded_frame.allocation);
  }
  recorded_swapchain_frames_.clear();
...
bool ImGuiVulkanRuntime::FrameRender(ImDrawData* draw_data) {
  ...
  if (result_texture_descriptor_set_ != VK_NULL_HANDLE && result_image_.valid()) {
    RecordResultPreviewCopy(fd->CommandBuffer);
  }
  ...
  if (swapchain_readback_enabled_) {
    if (result_texture_descriptor_set_ != VK_NULL_HANDLE && result_image_.valid()) {
      RecordResultPreviewCapture(fd->CommandBuffer);
    } else if (preview_renderer_ && preview_renderer_->HasTexture()) {
      RecordPreviewImageCapture(
        fd->CommandBuffer,
        preview_renderer_->PreviewTextureImage(),
        preview_renderer_->PreviewTextureExtent(),
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    } else {
      RecordSwapchainCapture(fd->CommandBuffer, fd->Backbuffer);
    }
  }
...
bool ImGuiVulkanRuntime::ReadbackRenderPreviewTexture(
  ...
  // The GUI uses result_texture_descriptor_set_ whenever it is available.
  // Read back that same displayed result image; do not capture the algorithm
  // image directly after the GUI has been given its own copy.
  if (result_texture_descriptor_set_ != VK_NULL_HANDLE &&
      result_image_.valid() &&
      result_preview_copy_.image != VK_NULL_HANDLE) {
    const VkExtent2D extent = result_preview_copy_.extent;
    ...
    to_transfer.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    to_transfer.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    to_transfer.image = result_preview_copy_.image;
    to_transfer.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
    to_transfer.subresourceRange.levelCount = 1u;
    to_transfer.subresourceRange.layerCount = 1u;
    vkCmdPipelineBarrier(
...
std::string ImGuiVulkanRuntime::RenderPreviewDebugSummary() const {
  ...
  if (pending_render_preview_request_.valid &&
      result_texture_descriptor_set_ != VK_NULL_HANDLE &&
      result_image_.valid()) {
    return
      "source=render_preview_window.result_image request=valid stage=" +
      pending_render_preview_request_.stage_name +
      ...
      " offscreen_surface=" +
      std::to_string(static_cast<uint32_t>(offscreen_surface_extent_.x)) + "x" +
      std::to_string(static_cast<uint32_t>(offscreen_surface_extent_.y)) +
      " vs=" + pending_render_preview_request_.vertex_shader_path +
      " fs=" + pending_render_preview_request_.fragment_shader_path;
  }
  return
    "source=render_preview_window.preview_renderer offscreen_surface=" +
    std::to_string(static_cast<uint32_t>(offscreen_surface_extent_.x)) + "x" +
    std::to_string(static_cast<uint32_t>(offscreen_surface_extent_.y)) + " " +
    preview_renderer_->DebugSummary();
}
...
void ImGuiVulkanRuntime::SetSwapchainReadbackEnabled(bool enabled) {
  swapchain_readback_enabled_ = enabled;
  debug_tool_recording_capable_ = enabled;
}

void ImGuiVulkanRuntime::BeginDebugToolRecording() {
  if (!debug_tool_recording_capable_) {
    throw std::runtime_error("DebugTools swapchain recording is not enabled for this runtime.");
  }
  if (debug_tool_recording_active_) {
    throw std::runtime_error("DebugTools swapchain recording is already active.");
  }
  if (!recorded_swapchain_frames_.empty()) {
    throw std::runtime_error("DebugTools swapchain recording buffer has not been taken.");
  }
  debug_tool_recording_active_ = true;
}

void ImGuiVulkanRuntime::EndDebugToolRecording() {
  if (!debug_tool_recording_active_) {
    throw std::runtime_error("DebugTools swapchain recording is not active.");
  }
  debug_tool_recording_active_ = false;
}

std::vector<DebugToolRecordedFrame> ImGuiVulkanRuntime::TakeDebugToolRecording() {
  if (debug_tool_recording_active_) {
    throw std::runtime_error("Stop DebugTools swapchain recording before taking its buffer.");
  }
  CheckVkResult(vkDeviceWaitIdle(device_));

  std::vector<DebugToolRecordedFrame> frames{};
  frames.reserve(recorded_swapchain_frames_.size());
  for (SwapchainCaptureFrame& recorded_frame : recorded_swapchain_frames_) {
    const VkDeviceSize size_bytes =
      static_cast<VkDeviceSize>(recorded_frame.width) *
      static_cast<VkDeviceSize>(recorded_frame.height) *
      static_cast<VkDeviceSize>(recorded_frame.bytes_per_pixel);
    CheckVkResult(vmaInvalidateAllocation(
      vma_allocator_,
      recorded_frame.allocation,
      0u,
      size_bytes));
    const auto* source_bytes =
      static_cast<const uint8_t*>(recorded_frame.allocation_info.pMappedData);
    const bool bgra =
      recorded_frame.format == VK_FORMAT_B8G8R8A8_UNORM ||
      recorded_frame.format == VK_FORMAT_B8G8R8A8_SRGB ||
      recorded_frame.format == VK_FORMAT_B8G8R8_UNORM ||
      recorded_frame.format == VK_FORMAT_B8G8R8_SRGB;
    DebugToolRecordedFrame frame{};
    frame.width = recorded_frame.width;
    frame.height = recorded_frame.height;
    frame.tick_sequence = recorded_frame.tick_sequence;
    frame.rgba.resize(static_cast<size_t>(recorded_frame.width) * recorded_frame.height * 4u);
    for (size_t pixel_index = 0u;
         pixel_index < static_cast<size_t>(recorded_frame.width) * recorded_frame.height;
         ++pixel_index) {
      const size_t source_index = pixel_index * recorded_frame.bytes_per_pixel;
      const size_t output_index = pixel_index * 4u;
      if (bgra) {
        frame.rgba[output_index + 0u] = static_cast<std::byte>(source_bytes[source_index + 2u]);
        frame.rgba[output_index + 1u] = static_cast<std::byte>(source_bytes[source_index + 1u]);
        frame.rgba[output_index + 2u] = static_cast<std::byte>(source_bytes[source_index + 0u]);
      } else {
        frame.rgba[output_index + 0u] = static_cast<std::byte>(source_bytes[source_index + 0u]);
        frame.rgba[output_index + 1u] = static_cast<std::byte>(source_bytes[source_index + 1u]);
        frame.rgba[output_index + 2u] = static_cast<std::byte>(source_bytes[source_index + 2u]);
      }
      frame.rgba[output_index + 3u] = recorded_frame.bytes_per_pixel == 4u
        ? static_cast<std::byte>(source_bytes[source_index + 3u])
        : std::byte{255};
    }
...
void ImGuiVulkanRuntime::RefreshResultTexture() {
  ...
  RemoveResultTexture();
  CreateResultPreviewCopy(image.extent);
