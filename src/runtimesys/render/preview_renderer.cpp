...
VkImage PreviewRenderer::PreviewTextureImage() const {
  return target_.image;
}
...
bool PreviewRenderer::UploadBuffer(
  ...
  if (buffer_resource->buffer == VK_NULL_HANDLE || buffer_resource->size_bytes < required_size) {
    ...
    buffer_info.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
      (source_buffer.draw_indirect ? VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT : 0u);
...
bool PreviewRenderer::CreatePipeline() {
  ...
  try {
    ...
    pipeline_.draw_indirect_buffer_index = UINT32_MAX;
    for (uint32_t buffer_index = 0u;
         buffer_index < static_cast<uint32_t>(request_.storage_buffers.size());
         ++buffer_index) {
      if (request_.storage_buffers[buffer_index].draw_indirect) {
        pipeline_.draw_indirect_buffer_index = buffer_index;
        break;
      }
    }
...
bool PreviewRenderer::Record(VkCommandBuffer command_buffer) {
  ...
  if (pipeline_.draw_indirect_buffer_index != UINT32_MAX) {
    vkCmdDrawIndirect(
      command_buffer,
      pipeline_.buffers[pipeline_.draw_indirect_buffer_index].buffer,
      0u,
      1u,
