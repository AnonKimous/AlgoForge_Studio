...
- `catalog`: internal implementation source group for package loading, plugin loading, support helpers, and intervention payload code
- `algomanager`: `algomanager/algorithm_manager.h` is the single public entrypoint
- `runtimesys`: `runtimesys/runtime_systems.h` is the public facade; `memory_manager.h` remains an internal support header
- `agentmanager`: `agentmanager/agent_management.h` is the public facade
...
- `SdlWindow` and `ImGuiVulkanRuntime` are internal implementation details of `runtimesys`.
...
- `agentmanager` coordinates mount/unmount/tick and forwards descriptor data to its managed agents, but it does not create containers itself.
- `sdk` talks directly to `agentmanager`, must not include `algomanager` directly, and should not depend on `debug_tool` frontend code.
- For the algorithm-management layer, the only public interface header is `algomanager/algorithm_manager.h`.
...
- Container manifests define storage tuple_width and scalar width, not semantic payload type. Payload interpretation belongs to package boundaries such as decomposer or reflector.
- For the runtimesys layer, the only public interface header is `runtimesys/runtime_systems.h`.
- For the agentmanager layer, the only public interface header is `agentmanager/agent_management.h`.
