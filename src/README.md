...
- `agent_management` sits above `algorithm_management` and owns the runtime agent object plus mount and submit entry points.
...
- `algorithm_catalog`: internal implementation source group for package loading, plugin loading, support helpers, and intervention payload code
...
- `agent_management` coordinates mount/unmount/tick and forwards descriptor data to its managed agents, but it does not create containers itself.
