...
std::filesystem::path _ResolvePipelineTimingExportDirectory() {
  ...
  const std::filesystem::path project_root =
    algomanager::ResolveProjectRootFromAlgorithmLibraryRoot(algorithm_library_root);
...
bool AgentManager::Tick(
  ...
  if (ticked_any_agent) {
    algomanager::RecordDebugToolTick();
  }
  return true;
}
