...
class PhysModeController {
 public:
  void SetPhysicsConfig(const PhysSolverConfig& config) { solver_.SetConfig(config); }
  void SetGpuComputeContext(const VulkanComputeContextView& context) { solver_.SetGpuContext(context); }
  void SetAlgorithmComplianceDescriptor(const AlgorithmComplianceDescriptor& descriptor) { solver_.SetAlgorithmComplianceDescriptor(descriptor); }
  ...
  const AlgorithmComplianceDescriptor& algorithm_compliance_descriptor() const { return solver_.algorithm_compliance_descriptor(); }
 ...
 private:
  ...
  service_domains::PhysManager solver_;
