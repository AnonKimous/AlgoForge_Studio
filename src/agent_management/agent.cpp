...
using PendingPipelineStage0Submission = algorithmManager::JobsPendingPipelineStage0Submission;
using PipelineLaneRuntimeState = algorithmManager::JobsPipelineLaneRuntimeState;
using PipelineRuntimeState = algorithmManager::JobsPipelineRuntimeState;
...
AlgorithmExecutionPreference _NormalizeExecutionPreference(
  ...
  bool vk_symbol,
  bool cuda_symbol,
  bool compatibility_symbol) {
  switch (requested_preference) {
    case AlgorithmExecutionPreference::Jobs:
      if (jobs_symbol) {
        return AlgorithmExecutionPreference::Jobs;
      }
      break;
    case AlgorithmExecutionPreference::Vk:
      if (vk_symbol) {
        return AlgorithmExecutionPreference::Vk;
      }
      break;
    case AlgorithmExecutionPreference::Cuda:
      if (cuda_symbol) {
        return AlgorithmExecutionPreference::Cuda;
      }
      break;
    case AlgorithmExecutionPreference::Compatibility:
      if (compatibility_symbol) {
        return AlgorithmExecutionPreference::Compatibility;
      }
...
bool _PreparePipelineStage0Submission(
  ...
  if (!algorithmManager::PrepareAlgorithmObjectByName(
...
void _AppendAlgorithmTimingLogLine(
  ...
  *stream << "algorithm " << object.algorithm_profile.algorithm_name
          ...
          << " | execution=" << (object.execution_preference == AlgorithmExecutionPreference::Jobs
            ? "jobs"
            : (object.execution_preference == AlgorithmExecutionPreference::Vk
              ? "vk"
              : (object.execution_preference == AlgorithmExecutionPreference::Cuda ? "cuda" : "compatibility")))
...
bool _TickAlgorithmObject(
  ...
  if (!algorithmManager::SubmitAlgorithmObject(
    ...
    std::cerr
      << "agent_submit_failed algorithm=" << object.algorithm_profile.algorithm_name
...
BuiltAlgorithmMount _BuildAlgorithmMount(
  ...
  if (!CreateAlgorithmObjectByName(
        algorithm_name,
        &object,
        &error_message,
        load_reflector)) {
  ...
  object.execution_preference = _NormalizeExecutionPreference(
    execution_preference,
    object.jobs_symbol,
    object.vk_symbol,
    object.cuda_symbol,
    object.compatibility_symbol);
  ...
  if (!algorithmManager::FinalizeAlgorithmObject(
        object,
...
bool CreateAlgorithmObjectByName(
  ...
  const bool created =
    algorithmManager::CreateAlgorithmObjectFromLocation(
      package_location,
      out_group,
      out_error_message,
      load_reflector);
...
bool QueryAlgorithmRequestedBindingsByName(
  ...
  if (!algorithmManager::QueryAlgorithmPackageRequestedBindingsFromLocation(
        package_location,
...
bool Agent::MountPipelineAlgorithm(
  ...
  return algorithmManager::MountPipelineAlgorithmObjects(
    &algorithm_objects_,
...
bool Agent::RemoveAlgorithm(size_t index) {
  ...
  const AlgorithmObject object_to_remove = algorithm_objects_[index];

  algorithm_objects_.erase(
    algorithm_objects_.begin() + static_cast<std::ptrdiff_t>(index));
  algorithm_runtime_states_.erase(
    algorithm_runtime_states_.begin() + static_cast<std::ptrdiff_t>(index));
  algorithm_assembly_states_.erase(
    algorithm_assembly_states_.begin() + static_cast<std::ptrdiff_t>(index));
  algorithmManager::UnregisterMountedPipelineObject(object_to_remove, agent_name_);
  return true;
}
...
void Agent::RefreshInterventionSignals(const AgentTickContext& context) {
  for (size_t i = 0; i < algorithm_objects_.size(); ++i) {
    ...
    algorithmManager::RefreshAlgorithmObjectSignals(
      algorithm_objects_[i],
      runtime_state,
      context);
  }
}
...
bool Agent::SubmitAlgorithm(
  ...
  auto process_algorithm_index = [&](size_t index) {
    ...
    std::string algorithm_error_message;
    if (!algorithmManager::TickAlgorithmObject(
          object,
          runtime_state,
          agent_name_,
          context,
          allow_tick,
          index < algorithm_assembly_states_.size()
            ? algorithm_assembly_states_[index]
            : AlgorithmAssemblyState::Failed,
          collect_timing_log,
          &algorithm_error_message)) {
      if (!algorithm_error_message.empty()) {
        std::cerr
          << "algorithm_tick_failed algorithm=" << object.algorithm_profile.algorithm_name
          << " error=" << algorithm_error_message << '\n';
      }
      runtime_state.algorithm_to_agent_signal.stop_requested = true;
    }
    if (collect_timing_log) {
      if (runtime_state.pipeline_stage_runtime_stats.empty()) {
...
bool Agent::EnqueuePipelineStage0Submission(
  ...
  return algorithmManager::EnqueueMountedPipelineStage0Submission(
    &algorithm_objects_,
    pipeline_name,
    agent_name_,
    resource_bindings,
...
bool Agent::ReplayPipelineStageBridgeDebug(
  ...
  const AgentTickContext& context,
  std::string* out_error_message) {
  if (index >= algorithm_objects_.size()) {
    if (out_error_message) {
      *out_error_message = "Selected algorithm object is unavailable.";
    }
    return false;
  }
  return algorithmManager::ReplayMountedPipelineDebugNode(
    &algorithm_objects_[index],
    &algorithm_runtime_states_[index],
    algorithm_objects_[index].pipeline_stage_debug_index,
    context,
    out_error_message);
}
...
void Agent::Destroy() {
  algorithmManager::UnregisterMountedPipelineObjects(algorithm_objects_, agent_name_);
  initialized_ = false;
  agent_name_.clear();
