...
This template shows how one agent can own a serial algorithm pipeline and redirect containers between steps.

## Example

Suppose a single render or simulation agent uses two algorithms:
...
The agent can keep one shared geometry container and redirect the per-step state containers as needed.

```cpp
AgentInitConfig config{};
...
- Build the agent-level descriptor by collecting the package descriptors, execution order, and container routes.
...
- Intervention packages can be attached per agent when UI, signal handling, or intervention delegation is needed; otherwise ordinary package codecs can stand on their own.
