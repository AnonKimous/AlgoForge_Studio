...
void AlgorithmManager::Init(const Mesh& mesh) {
  initial_mesh_ = mesh;
...
void AlgorithmManager::UpsertVelocityGuidance(const Mesh& mesh, int vertex, Vec3 requested_target) {
  UpsertVelocityGuidance(mesh, vertex, mesh.positions[vertex], requested_target);
}

void AlgorithmManager::UpsertVelocityGuidance(const Mesh& mesh, int vertex, Vec3 start, Vec3 requested_target) {
...
int AlgorithmManager::ClampDirectiveInsertionIndex(int index) const {
  if (index < 0) return 0;
  if (index > static_cast<int>(guidances_.size())) return static_cast<int>(guidances_.size());
  return index;
}

void AlgorithmManager::MoveVelocityGuidance(const Mesh& mesh, int from, int to) {
...
void AlgorithmManager::SetVelocityGuidanceHidden(int index, bool hidden) {
  if (index < 0 || index >= static_cast<int>(guidances_.size())) return;
  guidances_[index].hidden = hidden;
}

void AlgorithmManager::SetVelocityGuidances(const Mesh& mesh, const std::vector<VelocityGuidance>& guidances) {
  guidances_ = guidances;
  RevalidateDirectives(mesh);
}

void AlgorithmManager::SetGuideVelocities(const std::vector<VelocityGuideVelocity>& guide_velocities) {
  guide_velocities_ = guide_velocities;
}

void AlgorithmManager::SetGuideForces(const std::vector<VelocityGuideForce>& guide_forces) {
  guide_forces_ = guide_forces;
}

void AlgorithmManager::ApplyValidVelocityGuidances(Mesh& mesh, std::vector<VelocityMatrix>& total_velocities, std::vector<VelocityMatrix>& linear_velocities, std::vector<VelocityMatrix>& angular_velocities) {
...
void AlgorithmManager::RevalidateDirectives(const Mesh& mesh) {
...
PhysicsStepInput AlgorithmManager::BuildStepInput(const Mesh& mesh, const std::vector<VelocityMatrix>& total_velocities, const std::vector<VelocityMatrix>& linear_velocities, const std::vector<VelocityMatrix>& angular_velocities) const {
...
PhysicsStepOutput AlgorithmManager::Solve(const PhysicsStepInput& input) const {
...
bool AlgorithmManager::IsMeshStateAllowed(const std::vector<Vec3>& positions, const std::vector<PhysicsRestTriangle>& rest_triangles) const {
...
bool AlgorithmManager::IsTriangleAllowed(const std::vector<Vec3>& positions, const PhysicsRestTriangle& rest) const {
  const Vec3& o = positions[rest.origin];
  const Vec3& a = positions[rest.first];
  const Vec3& b = positions[rest.second];
...
Vec3 AlgorithmManager::FindNearestAllowedTarget(const std::vector<Vec3>& positions, const std::vector<PhysicsRestTriangle>& rest_triangles, int vertex, Vec3 start, Vec3 requested) const {
  Vec3 best = start;
  float lo = 0.0f;
  float hi = 1.0f;
  for (int i = 0; i < 20; ++i) {
...
void AlgorithmManager::ApplyCorotatedResponse(const PhysicsStepInput& input, PhysicsStepOutput& output, const std::vector<bool>& guided) const {
  (void)input;
  (void)output;
  (void)guided;
}
