...
4. The runtime request carries the final composite container descriptor for the entity.
5. Algorithm consumes only data that matches its own contract.
6. When an entity-level composite descriptor remaps container names, `AlgorithmMng_ResolveContainerName` resolves the final name that should be used for container lookup.
...
- `AlgorithmDataContract::container_aliases` is the package-scoped alias map used by entity-level composition.
