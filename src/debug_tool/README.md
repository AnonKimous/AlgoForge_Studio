...
- Backend-facing cross-layer calls are routed through the hooker modules in this folder.
- `debug_tool_hooker.h` is the single façade the backend includes.
...
- `debug_tool_hooks` owns the backend hooker surface so the backend stays orchestration-only, and `debug_tool_hooker.h` is the only backend-facing header.
