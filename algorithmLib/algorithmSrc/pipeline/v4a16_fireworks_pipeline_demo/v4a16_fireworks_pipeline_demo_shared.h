...
inline void _WriteDrawIndirect(
  algorithm::AlgorithmContainer* container,
  uint32_t vertex_count,
  uint32_t draw_count) {
  const uint32_t command[4] = {vertex_count, draw_count, 0u, 0u};
  std::memcpy(container->bytes.data(), command, sizeof(command));
}
...
class FireworksJobsExecutor final : public agent::IAlgorithmJobsExecutor {
 public:
  ...
  bool ExecuteJobsAlgorithm(
    ...
    algorithm::AlgorithmContainer* render_draw = _FindMutableContainer(algorithm_container_set, "render_draw");
    ...
    if (algorithm_name_ == "v4a16_fireworks_pipeline_demo_stageBegin") {
      ...
      const uint32_t launch_quota = std::min<uint32_t>(
        launch_budget_count,
        6u + static_cast<uint32_t>(std::floor(_Random01(tick_seed + 23u) * 5.0f)));
      uint32_t spawned_count = 0u;
      _WriteScalar(v1, tick_value + 1.0f);
      _WriteScalar(v3, live_spark_value);
      _WriteScalar(v4, static_cast<float>(_ArrayCount(spark_state)));
      if (shell_state && shell_pos_x && shell_pos_y && shell_vel_x && shell_vel_y && shell_age && shell_life && shell_seed) {
        const size_t shell_count = _ArrayCount(shell_state);
        const Vec2 launch_min = _PixelToLogical(
          Vec2{preview_extent.x * 0.04f, preview_extent.y * 0.05f},
          preview_extent);
        const Vec2 launch_max = _PixelToLogical(
          Vec2{preview_extent.x * 0.96f, preview_extent.y * 0.18f},
          preview_extent);
        const uint32_t emitter_columns = 6u;
        const uint32_t emitter_rows = 2u;
        for (size_t i = 0; i < shell_count; ++i) {
          ...
          const uint32_t emitter_column = spawned_count % emitter_columns;
          const uint32_t emitter_row = spawned_count / emitter_columns;
          const float emitter_u = (static_cast<float>(emitter_column) + 0.5f) / static_cast<float>(emitter_columns);
          const float emitter_v = (static_cast<float>(emitter_row) + 0.5f) / static_cast<float>(emitter_rows);
          const float x = launch_min.x +
            (emitter_u + (_Random01(tick_seed + static_cast<uint32_t>(i) * 17u) - 0.5f) * 0.06f) *
              (launch_max.x - launch_min.x);
          const float y = launch_min.y +
            (emitter_v + (_Random01(tick_seed + static_cast<uint32_t>(i) * 29u) - 0.5f) * 0.08f) *
              (launch_max.y - launch_min.y);
          const float vx = -0.018f + _Random01(tick_seed + static_cast<uint32_t>(i) * 31u) * 0.036f;
          const float vy = 0.058f + _Random01(tick_seed + static_cast<uint32_t>(i) * 47u) * 0.032f;
    ...
    } else if (algorithm_name_ == "v4a16_fireworks_pipeline_demo_stage1") {
      if (shell_state && shell_pos_x && shell_pos_y && shell_vel_x && shell_vel_y && shell_age && shell_life) {
        ...
        for (size_t i = 0; i < shell_count; ++i) {
          ...
          vy -= 0.0025f;
          age += 1.0f;
          const float explode_height = 0.48f + _Random01(seed_bits + static_cast<uint32_t>(i) * 7u + 13u) * 0.34f;
      ...
      _WriteScalar(v3, live_spark_value);
      _WriteScalar(v4, static_cast<float>(_ArrayCount(spark_state)));
    } else if (algorithm_name_ == "v4a16_fireworks_pipeline_demo_stage2") {
      if (shell_state && shell_pos_x && shell_pos_y && spark_state && spark_pos_x && spark_pos_y &&
        ...
        for (size_t i = 0; i < shell_count; ++i) {
          ...
          const float shell_age_value = 0.0f;
          const float shell_life_value = 4.0f;
          std::memcpy(shell_state->bytes.data() + i * shell_state->element_stride, &shell_state_value, sizeof(shell_state_value));
          std::memcpy(shell_age->bytes.data() + i * shell_age->element_stride, &shell_age_value, sizeof(shell_age_value));
          std::memcpy(shell_life->bytes.data() + i * shell_life->element_stride, &shell_life_value, sizeof(shell_life_value));
        }
      }
      _WriteScalar(v1, tick_value);
      _WriteScalar(v2, launch_budget_value);
      _WriteScalar(v3, live_spark_value);
      _WriteScalar(v4, static_cast<float>(_ArrayCount(spark_state)));
    } else if (algorithm_name_ == "v4a16_fireworks_pipeline_demo_stage3") {
      if (shell_state && shell_pos_x && shell_pos_y && shell_vel_x && shell_vel_y && shell_age && shell_life && shell_seed) {
        const size_t shell_count = _ArrayCount(shell_state);
        for (size_t i = 0u; i < shell_count; ++i) {
          float state = 0.0f;
    ...
    } else if (algorithm_name_ == "v4a16_fireworks_pipeline_demo_stage4") {
      algorithm::AlgorithmContainer* v4 = _FindMutableContainer(algorithm_container_set, "v4");
      ...
      _WriteScalar(v3, live_spark_value);
      _WriteScalar(v4, render_instance_count_value);
      _WriteDrawIndirect(render_draw, 4u, static_cast<uint32_t>(render_instance_count_value));
    }
