...
void main() {
  ...
  if (gl_VertexIndex == 0u && index == 0u) {
    ...
    for (uint i = 0u; i < kArrayLimit; ++i) {
      ...
      float vx = spark_vel_x_in.value[i] * 0.985;
      float vy = spark_vel_y_in.value[i] * 0.985;
      float life = spark_life_in.value[i] - 1.0;
      if (life > 0.01 && (abs(vx) + abs(vy)) > 0.01) {
        live_count += 1u;
      }
    }
    v3_out.value[0] = float(live_count);
  }
  ...
  if (gl_VertexIndex == 0u && index < kArrayLimit) {
    ...
    if (spark_state == 1.0) {
      float x = spark_pos_x_in.value[index] + spark_vel_x_in.value[index];
      float y = spark_pos_y_in.value[index] + spark_vel_y_in.value[index];
      float vx = spark_vel_x_in.value[index] * 0.985;
      float vy = spark_vel_y_in.value[index] * 0.985;
      float age = spark_age_in.value[index] + 1.0;
      float life = spark_life_in.value[index] - 1.0;
      float next_state = (life <= 0.01 || (abs(vx) + abs(vy)) <= 0.01) ? 0.0 : 1.0;
      spark_pos_x_out.value[index] = x;
      spark_pos_y_out.value[index] = y;
      spark_vel_x_out.value[index] = vx;
      spark_vel_y_out.value[index] = vy;
      spark_age_out.value[index] = age;
      spark_life_out.value[index] = life;
      spark_state_out.value[index] = next_state;
