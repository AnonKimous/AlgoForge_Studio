...
vec2 PixelToLogical(vec2 pixel) {
  return vec2(
    (pixel.x / algorithm_viewport.width) * 2.0 - 1.0,
    (pixel.y / algorithm_viewport.height) * 2.0 - 1.0
  );
}
...
void main() {
  ...
  uint tick_seed = uint(tick_value) + 1u;
  bool window_reset = (uint(tick_value) % 30u) == 0u;
  float launch_budget = window_reset ? 12.0 : launch_budget_value;
  uint launch_budget_count = uint(floor(launch_budget));
  uint launch_quota = min(
    launch_budget_count,
    6u + uint(floor(Random01(tick_seed + 23u) * 5.0))
  );
