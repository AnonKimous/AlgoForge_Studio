...
void main() {
  ...
  if (gl_VertexIndex == 0u && index < kArrayLimit) {
    ...
    if (shell_state <= 0.5) {
      ...
      float shell_age = shell_age_in.value[index];
      float shell_life = shell_life_in.value[index];
      if (shell_state > 2.5) {
        shell_age += 1.0;
        shell_life -= 1.0;
      }
      if (shell_life <= 0.01) {
        ...
        shell_age_out.value[index] = shell_age;
        shell_life_out.value[index] = shell_life;
    ...
    if (spark_state <= 0.5) {
      ...
      float spark_life = spark_life_in.value[index];
      if (spark_life <= 0.01) {
        spark_pos_x_out.value[index] = 0.0;
        spark_pos_y_out.value[index] = 0.0;
        spark_vel_x_out.value[index] = 0.0;
