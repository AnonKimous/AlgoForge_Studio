...
vec2 LogicalToPixel(vec2 logical_position) {
  return vec2(
    logical_position.x * algorithm_viewport.width,
    logical_position.y * algorithm_viewport.height
  );
}

vec2 LogicalVelocityToPixel(vec2 logical_velocity) {
  return vec2(
    logical_velocity.x * algorithm_viewport.width,
    logical_velocity.y * algorithm_viewport.height
  );
}
...
void main() {
  ...
  if (index < kArrayLimit) {
    ...
    if (shell_state > 0.5) {
      ...
      if (shell_life_in.value[index] > 0.01) {
    ...
    if (spark_state > 0.5) {
      ...
      if (spark_life_in.value[index] > 0.01) {
