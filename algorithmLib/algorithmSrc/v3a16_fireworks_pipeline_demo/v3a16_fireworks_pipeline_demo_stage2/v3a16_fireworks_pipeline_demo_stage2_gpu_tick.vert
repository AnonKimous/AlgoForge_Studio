...
void main() {
  ...
  if (index < kShellLimit) {
    ...
    if (state > 0.5 && state < 1.5) {
      ...
      float age = shell_age_in.value[index] + kDt;
      float life = shell_life_in.value[index];
      float explode_height = 0.42 + Hash3(float(index), v1_in.value[0], shell_seed_in.value[index]) * 0.38;
      float explode_bias = Hash3(float(index), v1_in.value[0] * 0.5, shell_seed_in.value[index] * 19.0);
  ...
  if (index >= kSparkStart) {
    float state = spark_state_in.value[index];
    uint owner_shell = (index - kSparkStart) / kSparkPerShell;
    uint local_index = (index - kSparkStart) % kSparkPerShell;
    if (state > 0.5) {
      float age = spark_age_in.value[index] + 1.0;
      float life = spark_life_in.value[index] - 1.0;
      float pos_x = spark_pos_x_in.value[index] + spark_vel_x_in.value[index];
      float pos_y = spark_pos_y_in.value[index] + spark_vel_y_in.value[index];
      float vel_x = spark_vel_x_in.value[index];
      float vel_y = spark_vel_y_in.value[index] * 0.985;
      bool expired = life <= 0.01;
      bool out_of_bounds =
        pos_x < -0.1 ||
        pos_x > 1.1 ||
        pos_y < -0.1 ||
        pos_y > 1.1;
      if (expired || out_of_bounds) {
        ...
        spark_pos_x_out.value[index] = pos_x;
        spark_pos_y_out.value[index] = pos_y;
        spark_vel_x_out.value[index] = vel_x;
        spark_vel_y_out.value[index] = vel_y;
        spark_state_out.value[index] = 1.0;
        spark_age_out.value[index] = age;
        spark_life_out.value[index] = life;
        spark_seed_out.value[index] = spark_seed_in.value[index];
      }
    } else if (shell_state_in.value[owner_shell] > 1.5) {
      float shell_x = shell_pos_x_in.value[owner_shell];
      float shell_y = shell_pos_y_in.value[owner_shell];
      float shell_seed = shell_seed_in.value[owner_shell];
      float angle = 6.2831853 * Hash3(float(index), v1_in.value[0], shell_seed);
      float radius = 0.008 + Hash3(float(index), shell_seed, v1_in.value[0]) * 0.038;
      float spark_speed = 0.010 + Hash3(float(local_index), shell_seed, v1_in.value[0] * 3.0) * 0.028;
      float spark_vx = cos(angle) * spark_speed;
      float spark_vy = sin(angle) * spark_speed;
      float spark_life = 42.0 + Hash3(float(index), shell_seed, float(local_index)) * 64.0;
      spark_pos_x_out.value[index] = shell_x + cos(angle) * radius;
      spark_pos_y_out.value[index] = shell_y + sin(angle) * radius;
      spark_vel_x_out.value[index] = spark_vx;
      spark_vel_y_out.value[index] = spark_vy;
      spark_state_out.value[index] = 1.0;
      spark_age_out.value[index] = 0.0;
      spark_life_out.value[index] = spark_life;
