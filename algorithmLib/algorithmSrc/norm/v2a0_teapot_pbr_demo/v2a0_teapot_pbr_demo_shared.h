...
namespace v2a0_teapot_pbr_demo {
...
constexpr size_t kMaxRenderTriangles = 256u;
...
inline bool BuildMeshCache(
  ...
  const algorithm::AlgorithmContainer* obj_container = FindConstContainer(container_set, "teapot_obj_bytes");
  const algorithm::AlgorithmContainer* mtl_container = FindConstContainer(container_set, "teapot_mtl_bytes");
  const algorithm::AlgorithmContainer* texture_container = FindConstContainer(container_set, "teapot_gold_tile_bytes");
  if (!obj_container || !mtl_container || !texture_container) {
    return false;
  }

  const uint64_t source_hash =
    HashBytes(obj_container->bytes.data(), obj_container->bytes.size()) ^
    (HashBytes(mtl_container->bytes.data(), mtl_container->bytes.size()) << 1u) ^
    (HashBytes(texture_container->bytes.data(), texture_container->bytes.size()) << 2u);

  static ParsedMeshCache cache;
  if (!cache.valid || cache.source_hash != source_hash) {
    AppendTeapotTrace("BuildMeshCache.parse.begin");
    const std::string obj_text = ReadTextContainer(obj_container);
    ...
    Vec3 min_value{};
    Vec3 max_value{};
    if (!ParseObj(obj_text, &triangles, &min_value, &max_value)) {
      return false;
    }
    AppendTeapotTrace("BuildMeshCache.parse.obj.end");
    ...
    if (triangles.size() <= kMaxRenderTriangles) {
      cache.triangles = std::move(triangles);
    } else {
      const size_t step = (triangles.size() + kMaxRenderTriangles - 1u) / kMaxRenderTriangles;
      cache.triangles.reserve(kMaxRenderTriangles);
      for (size_t i = 0u; i < triangles.size(); i += step) {
        cache.triangles.push_back(triangles[i]);
      }
    }
  ...
  bool ExecuteJobsAlgorithm(
    ...
    if (debug_state) {
      debug_state->signals.push_back(algorithm_management::AdvancedAlgorithmDebugSignal{
        .name = "v2a0_teapot_pbr_demo.jobs",
...
inline bool CreateBundle(
  ...
  out_bundle->reflector = is_stage1;
