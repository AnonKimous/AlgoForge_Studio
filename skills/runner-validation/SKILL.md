...
5. Inspect the newest CSV under `artifacts\pipeline_timing` when a pipeline is involved. Record each stage duration and look for stalled stages, timeout messages, or unusually large elapsed values.
...
Keep all runner logs, images, and temporary outputs under `testData\` or the repository's existing `artifacts\` directories. Do not write transient runner output at the repository root.
