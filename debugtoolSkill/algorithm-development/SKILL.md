...
```powershell
cmd /c build_algorithm_releaseWithDebugInfo.bat <algorithm-name>
```

For example:

```powershell
cmd /c build_algorithm_releaseWithDebugInfo.bat v4a16_fireworks_pipeline_demo
```

This script initializes Visual Studio, selects the repository LLVM/`clang-cl` toolchain, uses Ninja, builds shader and plugin artifacts, and invokes `algorithmLib\cache_algorithm_runtime.ps1` to produce the runtime package. This is the current verified algorithm build path. Read `debugtoolSkill\windows-path-environment\SKILL.md` first if the machine has the known PATH problem.
...
There is currently no checked-in, verified algorithm-specific MSVC batch file. `build_debugtool.bat` uses Visual Studio/MSBuild for `debugTool`; it is not the algorithm-package build command.

### Stale build references

`README.en.md` and the current Algorithm Studio build handler still refer to `build_algorithm.bat`. The repository root currently provides `build_algorithm_releaseWithDebugInfo.bat` instead. `algorithmDevTools\algorithm_studio\algorithm_studio.py` resolves the missing `build_algorithm.bat`, so do not claim that Algorithm Studio File -> Build is currently a valid build path. Use the dedicated batch above unless that separate UI issue has been fixed and verified.
...
Convenience entry points are:

```text
algorithmDevTools\run_debugtool_renderpreview.bat
algorithmDevTools\run_debugtool_preview_mount.bat <algorithm-name>
algorithmDevTools\launch_debugtool_cli.bat
```

The CLI and runner endpoint workflow is documented in `debugtoolSkill\debugtool-cli-runner\SKILL.md`. The preview-mount helper is a GUI convenience path; the runner is the reproducible validation path.
...
artifacts\pipeline_timing\<newest timing CSV>
```
...
Compare Jobs and Vulkan separately; do not infer visual frame rate from one CPU-side timing number. Use the CLI/debugTool performance commands in `debugtoolSkill\debugtool-cli-runner\SKILL.md` when a full pipeline report or CSV export is needed.
