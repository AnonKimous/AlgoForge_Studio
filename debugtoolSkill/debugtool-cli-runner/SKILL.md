...
Inspect `testData\pipeline\debugInfo\last_run.log` or `testData\norm\debugInfo\last_run.log`. For preview work require the exported file, nonzero `preview_pixels`, and a ready preview pipeline/target. Inspect the newest CSV in `artifacts\pipeline_timing` for stage timings. `OK` alone proves execution returned, not that a preview was visible.
