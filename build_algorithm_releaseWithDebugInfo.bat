...
"%CMAKE%" --build "%ALGO_BUILD_DIR%" --config %ALGO_BUILD_CONFIGURATION% --target %ALGO_PACKAGE_TARGETS% --parallel
if errorlevel 1 (
  set "EXITCODE=1"
  goto :cleanup
)
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%ALGO_ROOT%\package_algorithm_runtime.ps1" -AlgorithmSourceRoot "%ALGO_SRC_ROOT%" -AlgorithmRuntimeRoot "%ALGO_RUNTIME_ROOT%" -AlgorithmBuildConfiguration "%ALGO_BUILD_CONFIGURATION%"
if errorlevel 1 (
  set "EXITCODE=1"
  goto :cleanup
)
